---
title: "3_STIC_Nanostring_analysis"
...
top_50_count_data   <- count_data.epi[genes_top, , drop = FALSE]
high_var_count_data <- count_data.epi[apply(count_data.epi, 1, var, na.rm = TRUE) > 0.9, , drop = FALSE]
...
volcano_specs <- list(
  ...
  list(coef="Mixed_unique_STIC",         t1="Mixed STIC",         t2="Others",
       genes=c("MMP7","CP","CFB","MUC1","MUC16","MT1E","HLA-DRA","HLA-DRB1","HLA-DPB1",
               ...
               "H1-2","MT1X","CDK16","SOX4","SOX17","CLDN3","RPN1","FTH1", "COL6A2", 
               "CDH4","PRR23D2","HOXA3","FAM90A1","CHRNA4", "PLEKHH2"),
...
ggsave(
  filename = paste0(root, "/outputs/figures/combine_plot_chr.png"),
  plot = combined_plot,
  width = 16,
  height = 4.8,
  dpi = 300
...
ggsave(
  filename = paste0(root, "/outputs/figures/chr_all_combine_legend.png"),
  plot = final_plot,
  width = 16,
  height = 4.8,
  dpi = 300
...
# Save Heatmap
png(filename = paste0(root, "/outputs/figures/heatmap_molecular.png"), width =6400, height = 3200, res = 300)
ht <- Heatmap(
 scaled_genes,
 name = "Scaled\nExpression",
 col = col_fun,
...
Stromal_lymphocyte_colors <- c("Non-enriched"="#f7fcf0","Enriched"="#74c476")
...
ha2 <- HeatmapAnnotation(
  Diagnosis             = anno_data.epi$Diagnosis,
  `Stromal lymphocyte`  = anno_data.epi$Stromal_lymphocyte,  
...
png(file.path(root, "outputs/figures/heatmap_original.png"),
...
# Save to PNG
png(
 filename = file.path(root, "outputs/figures/heatmap_stroma.png"),
 width    = 6400,
 height   = 3200,
 res      = 300
)
...
```{r message=FALSE, warning=FALSE}
output_dir <- file.path(root,"outputs/figures/01_molecular_distance")
...
ggsave(filename = paste0(output_dir, "/Molecular_distance_re.png"),
...
distance_3d_plot <- plot_ly(
  ...
  add_trace(data = centers_md_3d,
            ...
            textfont = list(
              size = 16,
              color = dark_cluster_colors[as.character(centers_md_3d$`Molecular Subtype`)]
            )
  )%>%
  layout(
    paper_bgcolor = "white",
...
ggsave(filename = paste0(root, "/outputs/figures/02_sup_stroma/Stroma_HGSC_STIC.png"), plot = p1, width = 6, height = 6, dpi = 300)
```
